...
### DEV
- [ ] Define routing map for `quietcuration.xyz`: `/landing` = landing page, `/` = app login (note in brief). O
- [ ] Add `/landing` route shell in `quiet-curation-web` (layout + basic sections; copy can be stubbed). O
- [ ] Move login entry to `/` with `/login` redirect kept for compatibility; list `/login` references for follow-up updates (email deep links, auth redirects). O
